...
def update_everything(delta_time):
    global upProgress, upSpeed, frameCount_gameStart, score, game_state, x_transition, PLAY_AREA_CENTER
    screens()
...
def screens():
    ...
    draw_text("Restart", 3860, 192, color.WHITE, 20, font_name="Calibri",
              bold=True, italic=True)
    draw_text("Return", 250, 900, color.WHITE, 20, font_name="Calibri")
    draw_text(f"SCORE: {score}", 3860, 300, color.WHITE, 25, font_name="Calibri")
    # Instructions Screen
...
def mouse_detection(x, y, dx, dy):
    ...
    return_button_area = button_pos[6] - 100 <= x <= button_pos[6] + 100 and button_pos[7] - 50 <= button_pos[7] + 50

    # List for "for" loop
    button_area_list = [start_button_area, restart_button_area,
                        instruct_button_area, return_button_area]
